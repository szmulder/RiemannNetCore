...
namespace Riemann
{
    ...
    public class Client : IDisposable, IClient
    {
        ...
        public IDisposable Tick(int tickTimeInSeconds, string service, Func<TickEvent> onTick)
        {
            ...
            lock (_timerLock)
            {
                if (_ticks == null)
                {
                    ...
                    _timer = new Timer(_ => ProcessTicks(), null, TimeSpan.FromSeconds(1.0), TimeSpan.FromSeconds(1.0));
        ...
        private Response SendReceiveTcpMsg(Msg msg)
        {
            var response = new Response() { Ok = false };
        ...
        private Msg ReadReplyFromStream()
        {
            return Serializer.DeserializeWithLengthPrefix<Msg>(_tcpStream, PrefixStyle.Fixed32BigEndian);
        }
